...
class User(UserMixin, db.Model):
    ...
    # --- NEW: Profile Fields ---
    availability_prefs = db.Column(Text, nullable=True) # Store user's general availability text
    study_subjects = db.Column(Text, nullable=True) # Store comma-separated or newline-separated subjects
...
class Event(db.Model):
    ...
    __tablename__ = 'event'
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False, index=True) # Creator/Proposer
    subject = db.Column(db.String(200), nullable=False, index=True)
    type = db.Column(db.String(50), nullable=True, default='Study Session') # Default type
    proposed_time_string = db.Column(db.String(100), nullable=True) # Store original string if parsing fails
    ...
    reasoning = db.Column(db.Text, nullable=True) # Could be AI reasoning or manual note
...
def generate_event_image(subject):
    ...
    try:
        ...
        if existing_files:
            ...
            try:
                ...
                similarity_response = client.chat.completions.create( model="gpt-4o", messages=[ {"role": "system", "content": "You are an assistant comparing event subjects to existing image filename stems."}, {"role": "user", "content": f"{prompt_subject}\n\n{prompt_files}\n\n{prompt_instruction}"} ], temperature=0.2, max_tokens=100 )
                ...
                if match_result and match_result.lower() != 'none' and match_result in existing_files:
                    matched_filename = f"{match_result}.png"; relative_filepath = f"images/{matched_filename}"
                    print(f"Found semantically similar existing image: {relative_filepath}. Reusing.")
                    return relative_filepath
            except Exception as ai_err: print(f"ERROR: GPT-4o similarity check failed: {ai_err}. Proceeding to generate new image.")
    except Exception as file_err: print(f"ERROR: Could not list existing image files: {file_err}. Proceeding to generate new image.")
    safe_subject_base = sanitize_filename(subject)
    if not safe_subject_base: return None
    exact_filename = f"{safe_subject_base}.png"; exact_relative_filepath = f"images/{exact_filename}"; exact_absolute_filepath = IMAGES_FOLDER / exact_filename
    if exact_absolute_filepath.exists(): print(f"Image found for exact sanitized name '{safe_subject_base}': {exact_relative_filepath}"); return exact_relative_filepath
    ...
    try:
        ...
        response = client.images.generate( model="dall-e-3", prompt=dalle_prompt, size="1792x1024", quality="standard", n=1, response_format="url" )
        image_url = response.data[0].url; print(f"DALL-E generated image URL: {image_url}")
        new_absolute_filepath = IMAGES_FOLDER / f"{safe_subject_base}.png"; new_relative_filepath = f"images/{safe_subject_base}.png"
        image_response = requests.get(image_url, stream=True, timeout=30); image_response.raise_for_status()
        with open(new_absolute_filepath, 'wb') as f:
            for chunk in image_response.iter_content(chunk_size=8192): f.write(chunk)
        print(f"New image saved: {new_absolute_filepath}"); return new_relative_filepath
...
def run_ai_weekly_scheduling():
    ...
    try:
        ...
        users = User.query.filter(User.availability_prefs.isnot(None) | User.study_subjects.isnot(None)).all()
        if not users:
...
"""
        ...
        if "scheduled_events" in proposed_schedule and isinstance(proposed_schedule["scheduled_events"], list):
            ...
            for event_data in event_proposals:
                ...
                required_keys = ["subject", "type", "proposed_datetime", "proposed_location", "details", "reasoning", "involved_user_ids"]
                if not all(key in event_data for key in required_keys):
                    print(f"Warning: Skipping invalid event proposal from AI (missing keys): {event_data}")
                    continue

                proposed_dt_str = event_data.get('proposed_datetime')
                parsed_datetime = parse_datetime_string(proposed_dt_str)
                if not parsed_datetime:
                    print(f"Warning: Skipping event proposal with invalid datetime: {proposed_dt_str}")
                    continue

                # Simple check: Don't schedule in the past (relative to trigger time)
                if parsed_datetime < datetime.now(timezone.utc):
                     print(f"Warning: Skipping event proposal scheduled in the past: {proposed_dt_str}")
                     continue
                ...
                try:
                    ...
                    new_event = Event(
                        ...
                        reasoning=event_data['reasoning'] + f" | AI Matched Users: {event_data['involved_user_ids']}", # Add AI info
                        status='proposed'
                    )
                    db.session.add(new_event)
                    db.session.flush() # Get the ID for image generation pass
                    saved_event_ids.append({'id': new_event.id, 'subject': new_event.subject})
                    created_count += 1
                except Exception as e:
                    db.session.rollback()
                    print(f"ERROR creating event object from AI proposal: {e} - Data: {event_data}")

            if created_count > 0:
                 # Commit events first
                 db.session.commit()
                 print(f"Successfully committed {created_count} events generated by AI scheduler.")
                 ...
                 for event_info in saved_event_ids:
                     ...
                     print(f"Image assignment attempt for AI Event {event_id}, Subject: '{event_subject}'")
                     image_rel_path = generate_event_image(event_subject) # Uses the function with similarity check
                     if image_rel_path:
                         try:
                             ...
                             else: print(f"Warn: AI Event {event_id} not found for image update.")
                         except Exception as e: print(f"Error updating image filename for AI event {event_id}: {e}")

                 if num_images_assigned > 0:
                    try: db.session.commit(); print(f"Committed images/assignments for {num_images_assigned} AI events.")
                    except Exception as e: db.session.rollback(); print(f"ERROR: Commit image filenames failed for AI events: {e}")

            return created_count, f"AI scheduling complete. Proposed {len(event_proposals)} events, created {created_count}."
        else:
...
def create_event():
    if request.method == 'POST':
        ...
        errors = False
        if not subject: flash('Event subject is required.', 'error'); errors = True
        if not location: flash('Event location is required.', 'error'); errors = True
        if not datetime_str: flash('Event date and time are required.', 'error'); errors = True

        parsed_datetime = parse_datetime_string(datetime_str.replace(' ', 'T') + ':00Z') # Attempt basic ISO conversion
        if not parsed_datetime:
            # Try parsing with dateutil if available and basic fails
            if HAS_DATEUTIL:
                 try:
                     parsed_datetime = dateutil_parser.parse(datetime_str)
                     if parsed_datetime.tzinfo is None: # Assume UTC if no timezone provided
                          parsed_datetime = parsed_datetime.replace(tzinfo=timezone.utc)
                     else:
                          parsed_datetime = parsed_datetime.astimezone(timezone.utc) # Convert to UTC
                 except Exception:
                      flash('Invalid date/time format. Please use YYYY-MM-DD HH:MM or a recognizable format.', 'error'); errors = True
            else:
                 flash('Invalid date/time format. Please use YYYY-MM-DD HH:MM format.', 'error'); errors = True
